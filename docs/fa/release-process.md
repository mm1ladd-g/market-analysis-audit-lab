...
۷. بازبینی حقوق، حریم، امنیت و روش را تکمیل و یک Tag حاشیه‌نویسی‌شده با قالب `vMAJOR.MINOR.PATCH` روی `main` ایجاد کنید. Workflow نسخه را با `pyproject.toml` تطبیق می‌دهد و Archive مبدأ، بستهٔ Python، Image چندمعماری نسخه‌دار، SBOM وابستگی و Container، فهرست مجوز، گزارش آسیب‌پذیری، Attestation منشأ و SHA-256 را می‌سازد. هرگز فایل `.env` یا خروجی فضای کاری را به Release پیوست نکنید.
