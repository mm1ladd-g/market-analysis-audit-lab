# Documentation

[English](index.md) · [فارسی](../fa/index.md) · [Repository](../../README.md)

Market Analysis Audit Lab is a local-first evidence pipeline for authorized market-analysis content. Start with the synthetic demo; configure a real source only after reviewing rights, privacy, cost, and methodology.

This repository is the generic reusable engine, not a distributable copy of any branded audit instance. Named-subject evidence, videos, transcripts, portraits, logos, licensed market data, real results, and real-instance domain, tunnel, credential, and private deployment configuration remain outside the repository under the operator's control.

## Operate

- [Quick start](quickstart.md) and [installation](installation.md)
- [Configuration](configuration.md)
- [Stage workflow and runbook](workflow.md)
- [Troubleshooting](troubleshooting.md)
- [Deployment](deployment.md)

## Understand the evidence

- [Architecture](architecture.md)
- [Methodology and limitations](methodology.md)
- [Claim and scoring model](claim-and-scoring-model.md)
- [Transcripts and language](transcripts-and-language.md)
- [Market data](market-data.md)
- [OpenAI models, data flow, and cost](openai-models-and-cost.md)
- [Artifacts and reproducibility](artifacts-and-reproducibility.md)
- [Outputs and dashboard](outputs-and-dashboard.md)

## Publish responsibly

- [Legal and source rights](legal-and-rights.md)
- [Fairness, corrections, and right of reply](fairness-and-publication.md)
- [Privacy](privacy.md) and [security](security.md)
- [Release process](release-process.md)

## Build and contribute

- [Development and testing](development-and-testing.md)
- [FAQ and glossary](faq-and-glossary.md)
- [Contributing](../../CONTRIBUTING.md), [governance](../../GOVERNANCE.md), and [roadmap](../../ROADMAP.md)

Documentation describes the tagged release. When code and documentation differ, stop and open an issue; do not silently infer a safer or more favorable audit behavior.
